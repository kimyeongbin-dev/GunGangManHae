...
| `0008_blind_hardening.sql` | Realtime 누수 차단(publication 제거 + 신호 트리거) + 팀장 딜량·소갯말 가리는 공개 뷰 |
| `0009_view_readonly.sql` | ⚠️ 0008 뷰의 쓰기 권한 회수(anon 이 뷰로 지명을 조작할 수 있던 구멍 봉인) |
| `0010_audit2_fixes.sql` | 티어 초기화/랜덤배치 시 지그재그 방향 소급 뒤집힘 수정(draft_order tombstone) + 결과공개창 비팀장 딜량·소갯말도 은닉 |
...
- **딜량·소갯말 은닉** — 실명이 공개되는 대상과 같은 행에 딜량이 보이면 '실명 ↔ 딜량' 지문이
  생겨 다음 판에서 익명 카드를 역추적할 수 있다(딜량은 회전 대상이 아니라 영구 지문). 그래서
  `participants_public` 뷰가 (a) 팀장은 항상, (b) 결과 전체공개 중에는 비팀장까지 딜량·소갯말을
  `null` 로 가린다. 결과 공개는 드래프트가 끝난 뒤라 그 순간 딜량이 필요 없다.

### 운영 시 주의

`intro`(소갯말)는 뽑기 판단에 필요해 비팀장에게는 공개된다. 참가자가 소갯말에 자신을
특정할 수 있는 내용을 쓰면 블라인드가 약해지므로, 등록 단계에서 안내가 필요하다.
...
-- 팀장의 딜량이 가려지는지 (팀장 행의 count(avg_damage) 가 0 이어야 정상)
select is_leader, count(*), count(avg_damage) from participants_public group by is_leader;
```
